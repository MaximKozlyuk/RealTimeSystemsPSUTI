...
void roundRobinConfiguration () {
    /* T_RRB causes the current task to be marked as undergoing the round-robin scheduling policy. */
    rt_task_set_mode(0,T_RRB ,NULL);
    rt_task_slice(NULL, 1);
}
...
void demo(void *arg) {
    ...
    while(runtime < EXECTIME) {
        rt_timer_spin(SPINTIME * BASEPERIOD);
        runtime += SPINTIME;
